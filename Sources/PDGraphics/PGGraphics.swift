...
let PGDisplayWidth = Display.width
let PGDisplayHeight = Display.height

let PGDisplayWidthF = Float(Display.width)
let PGDisplayHeightF = Float(Display.height)

/// A typealias representing a color.
public typealias PGColor = Graphics.Color
...
public func PGDrawScanline(_ x0: Int, _ x1: Int, y: Int, color: PGColor = .black, into frameBuffer: inout PGBuffer) {
    if case .solid(.clear) = color {
        PDReportWarning("Drawing a clear line does nothing.")
        return
    }
    ...
    if end < 0 || start >= PGDisplayWidth {
        return
    }

    start = max(start, 0)
    end = min(end, PGDisplayWidth - 1)
    ...
    @unknown default:
        PDReportError("An unsupported color type was provided to PGDrawScanline: \(color)")
        return
    }
